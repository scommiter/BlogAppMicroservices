...
namespace Chat.Api.EventBusConsumer
{
    public class NotificationConsumer : IConsumer<NotificationEvent>
    {
        ...
        {
            var connections = await _presenceTracker.GetConnectionsForUser(context.Message.Sender);
            if (connections != null)
            {
                await _presenceHub.Clients.Clients(connections).SendAsync("NewNotificationReceived", context.Message.Commentator);
